    ...
    <div className="flex min-h-screen flex-col bg-[#F7F7F9]">
      ...
      <header className="sticky top-0 z-40 border-b border-[#e8e8ec] bg-white">
        <div className="mx-auto flex h-14 w-full max-w-[920px] items-center px-4 sm:px-6 lg:h-[72px] lg:px-0">
          <button
            onClick={() => router.back()}
            className="text-[14px] text-[#666] px-1"
            disabled={publishing}
          >
            取消
          </button>
          ...
          <button
            onClick={publish}
            disabled={publishing}
            className="interactive-press rounded-full bg-[#FB7299] px-5 py-1.5 text-[14px] font-medium text-white transition-colors duration-150 hover:bg-[#ee628b] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {publishing ? '发布中' : '发布'}
          </button>
        </div>
      ...
      <div className="mx-auto w-full max-w-[920px] lg:my-6 lg:rounded-[18px] lg:border lg:border-[#e7e7eb] lg:bg-white lg:px-5 lg:py-2 lg:shadow-[0_10px_30px_rgba(27,27,38,0.035)]">
      {/* 标题 */}
      <div className="px-4 pt-4 lg:px-3">
        <input
          ...
          placeholder="标题（必填）"
          aria-label="帖子标题"
          className="w-full text-[16px] font-medium text-[#2C2C2C] outline-none placeholder:text-[#929292] pb-3 border-b border-gray-100"
        />
      </div>
      ...
      <div className="px-4 py-3 lg:px-3">
        <div
          ...
          aria-label="帖子正文"
          aria-multiline="true"
          className="min-h-[200px] text-[15px] text-[#2C2C2C] leading-[1.8] outline-none [&_img]:inline-block [&_img]:align-middle"
        ...
        <div className="grid grid-cols-3 gap-2">
          {images.map((img, i) => (
            <div key={i} className="relative">
              <img
                ...
                alt=""
                className="w-full h-24 rounded-[12px] object-cover bg-[#F7F7F9]"
              />
              <button
          ...
          {images.length < 9 && (
            <button
              ...
              className="w-full h-24 rounded-[12px] bg-[#F7F7F9] flex flex-col items-center justify-center gap-1 text-[#929292]"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#929292" strokeWidth="2" strokeLinecap="round">
      ...
      <div className="mt-4 px-4 lg:px-3">
        <p className="text-[12px] text-[#929292] mb-2">选择板块</p>
          ...
          {PLATES.map((p) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-[13px] font-medium transition-colors ${
                plate === p.id ? 'bg-[#FB7299] text-white' : 'bg-[#F7F7F9] text-[#666]'
              }`}
            >
              {p.name}
            </button>
      ...
      <button
        ...
        className="interactive-press mx-4 mb-3 mt-5 flex h-10 w-10 items-center justify-center rounded-full bg-[#F7F7F9] text-[18px]"
        aria-label="表情"
      >
      ...
      {/* 表情选择器 */}
      {showEmoji && (
        <div className="mx-4 mb-4 rounded-[16px] bg-[#F7F7F9] p-3">
