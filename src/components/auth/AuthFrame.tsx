...
export default function AuthFrame({ title, description, children, footer }: AuthFrameProps) {
  return (
    <div className="min-h-screen bg-[#F7F7F9] text-[#2C2C2C]">
      <header className="border-b border-[#e8e8ec] bg-white/95">
        <div className="mx-auto flex h-16 w-full max-w-[1080px] items-center justify-between px-5 sm:px-8">
          ...
          <Link href="/" className="text-[13px] text-[#929292] transition-colors hover:text-[#FB7299]">
            返回首页
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto grid w-full max-w-[1080px] gap-8 px-5 py-8 sm:px-8 lg:min-h-[calc(100vh-64px)] lg:grid-cols-[minmax(0,1fr)_420px] lg:items-center lg:gap-16 lg:py-12">
        <section className="hidden lg:block">
          <div className="max-w-[420px]">
            <p className="text-[13px] font-semibold tracking-[0.12em] text-[#FB7299]">杯友酱社区</p>
            ...
            <div className="mt-10 space-y-4 border-l border-[#f6b4c3] pl-4 text-[13px] text-[#666]">
              <p>发布自己的体验与问题</p>
              <p>参与板块讨论和回复</p>
              <p>同步个人内容与互动</p>
            </div>
        ...
        <section className="w-full max-w-[420px] justify-self-center rounded-[24px] border border-[#e7e7eb] bg-white p-6 shadow-[0_16px_40px_rgba(27,27,38,0.06)] sm:p-8">
          <div>
            <h1 className="text-[22px] font-bold tracking-[-0.02em] text-[#202024]">{title}</h1>
            <p className="mt-2 text-[13px] text-[#929292]">{description}</p>
          </div>
